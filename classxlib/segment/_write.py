...
def write_segment_image(
    ...
    try:
        ...
        segment_numbers, segment_area_count = np.unique(
            segment_data, return_counts=True
        )
        ...
        segment_h5_file.create_dataset(
            name=segment_dataset_name, data=segment_data, dtype=datatype
        )

        # Creating the segment info dataset
        segment_h5_file.create_dataset(
            name=segment_info_dataset_name, data=segment_info, dtype=datatype
        )
...
def update_segment_image_info(
    segment_info: np.ndarray, path: str, segment_info_dataset_name: str = "segment_info"
) -> bool:
    ...
    try:
        ...
        # Returns True to signify successful write
        return True
    except (OSError, RuntimeError, ValueError, TypeError, FileNotFoundError) as error:
        print("Error:", error)
        traceback.print_tb(error.__traceback__)
        return False
