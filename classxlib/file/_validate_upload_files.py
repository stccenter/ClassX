...
def validate_upload_files(
    ...
    try:
        for file in file_upload_list:
            ...
            if (
                original_image_service.check_duplicate_user_image(
                    user_id, upload_file_name
                )
                is True
            ):
            ...
            if upload_file_name.endswith(domain_data_protocol["file_type"]) is False:
                print("Invalid File detected (file type): %s", upload_file_name)
                file_list["invalid"].append(upload_file_name)
                continue
            ...
            # Begin checking and validating the file
                ...
                file_name_fields = upload_file_name.removesuffix(
                    domain_data_protocol["file_type"]
                )
                ...
                if file_name_fields[0] != domain_data_protocol["file_head"]:
                    print("Invalid File detected (Check if the beginning of the file's name matches what's expected): %s", upload_file_name)
                    file_list["invalid"].append(upload_file_name)
                    continue
                # Iterate based off the number of fields that need to be checked
                for field in range(validation_fields["num_fields"]):
                    print(
                        "Field being checked: %s & Existing field: %s",
                        field,
                        validation_fields,
                    )
