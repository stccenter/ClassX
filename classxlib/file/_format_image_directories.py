...
def format_image_directories(
    dir_: str, file_name: str, file_head: str, file_type: str
) -> dict:
    ...
    try:
        ...
        visual_image_savepath = merge_directory(
            dir_, file_name.replace(file_type, ".png")
        )

        # Save path for the normalized image data
        h5_savepath = visual_image_savepath.replace(file_head, ("h5_" + file_head))
        h5_savepath = h5_savepath.replace(".png", ".h5")

        # Save path for the thumbail image
        thumbnail_savepath = visual_image_savepath.replace(
            file_head, ("thumbnail_" + file_head)
        )

        # Save path for the image with gridlines drawn
        grid_savepath = visual_image_savepath.replace(file_head, ("grid_" + file_head))

        # Dictionary to return all paths in
        file_path_dict = {
            "visual": visual_image_savepath,
            "h5": h5_savepath,
