"""Util functions for classifying images."""
...
def classify_image(
    ...
    if model == 0:
        ...
        if needs_retrain is True:
            ...
            # Loading prexisting model
            classifer = joblib.load(saved_model_file_path)
    classified_block = classify_block(
        input_image,
        segment_image,
        image_type,
        image_domain,
...
def classify_block(
    ...
    # TODO: Finish docs
    """Classify a block of segments.

    Args:
        image_block (_type_): _description_
        segment_image (_type_): _description_
        image_type (_type_): _description_
        image_date (_type_): _description_
        classifer (_type_): _description_
        wb_ref (_type_): _description_
        bp_ref (_type_): _description_
        prob_threshold (_type_): _description_
    ...
    if image_type == "wv02_ms":
        input_feature_matrix = attr_calc.analyze_ms_image(
            image_block, segment_image, wb_ref, bp_ref
        )
    ...
    segment_predictions = np.ndarray.astype(
        segment_predictions, dtype=c_int, copy=False
    )
