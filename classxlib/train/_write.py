"""Module for writing training files"""
...
from ..database.model import SegmentImage, CropImage, OriginalImage
...
def write_training_file(
    ...
    label_vector, feature_matrix, segment_id_link = _prepare_dataset(
    ...
    if overwrite is True:
        ...
        _write_append_training_dataset(
            save_path=save_path,
...
def _calc_attributes(original_image, secondary_image, segment_id):
    ...
    feature_array = attr_calc.analyze_srgb_image(
        original_image, secondary_image, segment_id=segment_id
    )
...
def _prepare_dataset(
    ...
    segment_image_path = merge_directory(base_directory, segment_image_obj.segment_path)
    ...
    if crop_image_obj.h5_path is not None:
        ...
        crop_image_path = merge_directory(
            base_directory, crop_image_obj.visualization_path
        )
...
def _get_feature_matrix(crop_image, segment_image, segment_info, segment_count):
    ...
    if segment_count > 1000:
        with Pool(int(os.cpu_count() / 2)) as pool:
            func = partial(_calc_attributes, crop_image, segment_image)
        ...
        for segment in segment_info[:, 0].tolist():
            feature_matrix.append(
                _calc_attributes(crop_image, segment_image, segment_id=segment)
            )
    feature_matrix = np.array(feature_matrix)
    return feature_matrix


def _write_new_training_dataset(
    save_path: str,
    ...
    with h5py.File(save_path, "w") as training_file:
        training_file.create_dataset(
            "feature_matrix", data=feature_matrix, chunks=True, maxshape=(None, None)
        )
        training_file.create_dataset(
            "srgb", data=label_vector, chunks=True, maxshape=(None,)
        )
        training_file.create_dataset(
            "segment_id_link", data=segment_id_link, chunks=True, maxshape=(None, None)
        )
...
def _write_append_training_dataset(
    save_path: str,
    ...
    with h5py.File(save_path, "a") as training_file:
        ...
        if np.any(segment_id_link[:, 1] == segment_image_id):
            print(
                "Training image already exists in the current file replacing with new data."
            )
            ...
            for variable_name in ["feature_matrix", "srgb", "segment_id_link"]:
                if variable_name in training_file:
                    ...
                    print(original_dataset.shape, new_dataset.shape)
                    combined_dataset = np.concatenate((original_dataset, new_dataset))
                    del training_file[variable_name]
                    training_file.create_dataset(
                        variable_name, data=combined_dataset, chunks=True
                    )
        else:
            for variable_name in ["feature_matrix", "srgb", "segment_id_link"]:
                if variable_name in training_file:
