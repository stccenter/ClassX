...
def extract_label_mask_from_image(
    ...
    try:
        ...
        for label_id in label_id_list:
            ...
            if export:
                mask = np.isin(
                    segment_image,
                    np.where(segment_info[:, 0] == label_id, segment_info[:, 1], 0),
                )
            else:
                mask = np.isin(
                    segment_image,
                    np.where(segment_info[:, 1] == label_id, segment_info[:, 0], 0),
                )
...
def export_image_file_type(
    crop_image_path: str, export_image_dir: str, new_file_type: str
) -> None:
...
def export_mask_file_type(
    label_mask: np.ndarray, export_mask_dir: str, new_file_type: str
) -> None:
    ...
    try:
        match new_file_type:
            ...
            case _:
                ...
                if new_file_type not in ["png_8", "jpg"]:
...
def export_as_coco(
    ...
    Args:
        annotation_id (int): Id of annotations in the COCO file.
        crop_image_id (int): Id of related image within the annotation.
...
def export_solar(original_map:sunpy.map,
...
def _create_sub_masks(label_mask:np.ndarray,
    ...
    Args:
        ...
        annotation_id (int, optional): Required for COCO, id of each annotation. Defaults to None.
        create_coco (bool, optional): Flag to create a COCO file. Defaults to False.
        crop_image_id (int, optional): Required for COCO, image id (used to relate annotations and images). Defaults to None.
    ...
    try:
        ...
        for label_id in unique_label_id_list:
            ...
            # Wherever the mask is equal to True it puts the label id of the pixel
            sub_masks[label_id] = np.where(
                label_mask == label_id, label_id, sub_masks[label_id]
            )
            ...
            if create_coco and label_id != 0:
                ...
                master_dict["annotations"].append({
                    "id": int(annotation_id[0]),
                    "image_id": int(crop_image_id),
