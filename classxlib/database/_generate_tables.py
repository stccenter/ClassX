...
def generate_tables(engine: Engine) -> bool:
    ...
    """
    try:
        # Creating a shared registry object for generating the tables
        mapper_registry = registry()

        # Mapping out all the table structures
        _map_registry(mapper_registry)
...
def _map_registry(registry: registry):
    ...
    registry.map_imperatively(
        ...
        user_table,
        properties={
            "original_images": relationship(
                OriginalImage,
                primaryjoin=user_table.c.id == original_image_table.c.user_id,
            ),
            "crop_images": relationship(
                CropImage, primaryjoin=user_table.c.id == crop_image_table.c.user_id
            ),
            "segment_images": relationship(
                SegmentImage,
                primaryjoin=user_table.c.id == segment_image_table.c.user_id,
            ),
            "label_images": relationship(
                LabelImage, primaryjoin=user_table.c.id == label_image_table.c.user_id
            ),
            "training_files": relationship(
                TrainingFile,
                primaryjoin=user_table.c.id == training_file_table.c.user_id,
            ),
            "user_research_fields": relationship(
                UserResearchField,
                primaryjoin=user_table.c.id == user_research_field_table.c.user_id,
            ),
            "user_friends": relationship(
                UserFriend, primaryjoin=user_table.c.id == user_friend_table.c.sender_id
            ),
            "user_friends": relationship(
                UserFriend,
                primaryjoin=user_table.c.id == user_friend_table.c.receiver_id,
            ),
        },
    )
    ...
    registry.map_imperatively(
        ...
        properties={
            "users": relationship(
                User, primaryjoin=user_level_table.c.id == user_table.c.user_level
            )
        },
    )

    # Mapping out relationships for User Friends table
    registry.map_imperatively(UserFriend, user_friend_table)

    # Mapping out relationships for User research field table
    registry.map_imperatively(UserResearchField, user_research_field_table)
    ...
    registry.map_imperatively(
        ...
        properties={
            "crop_images": relationship(
                CropImage,
                primaryjoin=original_image_table.c.id == crop_image_table.c.original_image_id,
            )
        },
    )
    ...
    registry.map_imperatively(
        ...
        properties={
            "segment_images": relationship(
                SegmentImage,
                primaryjoin=crop_image_table.c.id == segment_image_table.c.crop_image_id,
            )
        },
    )
    ...
    registry.map_imperatively(
        ...
        properties={
            "label_images": relationship(
                LabelImage,
                primaryjoin=segment_image_table.c.id == label_image_table.c.segment_image_id,
            )
        },
    )

    # Mapping out relationships for Label Images table
    registry.map_imperatively(LabelImage, label_image_table)
    ...
    registry.map_imperatively(
        ...
        properties={
            "label_images": relationship(
                LabelImage,
                primaryjoin=training_file_table.c.id == label_image_table.c.training_file_id,
            )
        },
    )
    ...
    registry.map_imperatively(
        ResearchField,
        research_field_table,
        properties={
            "original_images": relationship(
                OriginalImage,
                primaryjoin=research_field_table.c.id == original_image_table.c.research_id,
            ),
            "crop_images": relationship(
                CropImage,
                primaryjoin=research_field_table.c.id == crop_image_table.c.research_id,
            ),
            "segment_images": relationship(
                SegmentImage,
                primaryjoin=research_field_table.c.id == segment_image_table.c.research_id,
            ),
            "training_files": relationship(
                TrainingFile,
                primaryjoin=research_field_table.c.id == training_file_table.c.research_id,
            ),
            "user_research_fields": relationship(
                UserResearchField,
                primaryjoin=research_field_table.c.id == user_research_field_table.c.research_id,
            ),
        },
    )
