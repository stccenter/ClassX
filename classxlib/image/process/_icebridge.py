...
def _process_icebridge_image(path: str, research_field_obj: ResearchField):
    ...
    (image_data, image_metadata, image_coordinates) = read_geotiff_image(
        path=path,
        reverse_axis=True,
    ...
    light_value = get_image_light(np.copy(image_data), data_range=(0, 255))

    # Processing the image metadata
    metadata_dict, creation_date = _process_metadata(
    ...
    image_data_dict["h5"] = rescale_intensity(
        image_data, old_range=(0, 255), new_range=(0.0, 1.0)
    )
...
def _color_balance_image(input_image: np.ndarray, channel_axis: int = None):
    ...
    for band_index in range(band_count):
        ...
        band_data = get_channel(
            input_image=input_image, channel=band_index, channel_axis=channel_axis
        )
        ...
        peaks = peak_local_max(
            ...
            min_distance=5,
            num_peaks=3,
            threshold_abs=int(np.sum(band_histogram[0]) * 0.004),
        )

        # Getting the lower and upper ranges
        lower, upper = _find_threshold(
            band_histogram, bin_centers=bin_centers, peaks=peaks, src_dtype=8
        )
        ...
        mask = np.where(band_data > 0, True, False)

        # Clipping the band data to the lower and upper ranges
        band_data = np.clip(band_data, a_min=lower, a_max=upper, where=mask)

        # Rescaling the band data
        band_data = rescale_intensity(
            band_data,
...
def _process_metadata(image_metadata, image_coordinates, image_light_value):
    ...
    gpsdatetime = datetime.strptime(
        image_metadata["GPSDate"] + " " + image_metadata["GPSTime"][0:8],
        "%Y-%m-%d %H:%M:%S",
    )
