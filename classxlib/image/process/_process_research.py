...
def process_research_image(
    image_path: str,
    image_savepath: str,
    file_name: str,
    research_field_obj: ResearchField,
):
    """Takes an image filepath from a default research field and processes it.
    Function requires the research field
    ...
    Args:
        ...
        research_field_obj (ResearchField) : Database research field object used
...
def _process_default_image(
    ...
    if research_field_obj.name == "Arctic Ice":
        image_data_dict, image_dim, creation_date, metadata_dict = (
            _process_icebridge_image(image_path, research_field_obj)
        )
        image_savepath_dict = format_image_directories(
            dir_=image_savepath, file_name=file_name, file_head="DMS", file_type=".tif"
        )

    elif research_field_obj.name == "Heliophysics":
        image_data_dict, image_dim, creation_date, metadata_dict = (
            _process_heliophysic_image(image_path, research_field_obj)
        )
        image_savepath_dict = format_image_directories(
            dir_=image_savepath, file_name=file_name, file_head="aia", file_type=".fits"
        )
    else:
        return None
    ...
    image_db_path_dict["thumbnail"] = format_database_path(
        image_savepath_dict["thumbnail"]
    )
