...
class oAuthManager:
    ...
    def __init__(self, client_id, client_secret) -> None:
        ...
        self.__FLASK_ENV = environ.get("FLASK_ENV")
        ...
        if not self.__BASE_URL or not self.__REALM:
            raise EnvironmentError("Could not find envoriment variables for keycloak's base url & the realm to manage.")
        # Ensure we have a trailing slash to make life easier
        elif not self.__BASE_URL.endswith("/"):
            self.__BASE_URL += "/"
    ...
    def __get_auth_token(self):
        ...
        try:
            print("REQUEST URL")
            print(f"{self.__BASE_URL}realms/{self.__REALM}/protocol/openid-connect/token")
            print("DATA")
            data = {
        ...
        data = res.json()
        self.__token = {"token": data["access_token"], "expires": datetime.datetime.now() + datetime.timedelta(0, data['expires_in'])}
    ...
    def __get_keycloak_cert(self):
        ...
        res = self.__session.get(f"{self.__BASE_URL}realms/{self.__REALM}", headers={'Authorization': 'Bearer ' + self.__token['token']} )

        self.__certificate = {"cert": "-----BEGIN PUBLIC KEY-----\n" + res.json()['public_key'] + "\n-----END PUBLIC KEY-----", "expires": datetime.datetime.now() + datetime.timedelta(.5) }

    def __validate_current_auth_token(self):
        """
        Private method to get a new auth token if our current one is expired private to not clog intelisense
    ...
    def __validate_current_cert(self):
        """
        Private method to get a new cert if our current one is expired private to not clog intelisense
    ...
    def validate_user_session(self) -> bool:
        """
        Validate a user's current session against keycloak this method just simplifies getting the access token
        ...
        try:
            if 0 < time_until_expires < 120: # Refresh the token if <2min remaining but not expired
    ...
    def decode_token(self, token: str) -> dict:
        ...
        try:
            decoded: dict = jwt.decode(token, self.__certificate["cert"], ["RS256"], audience="account")
    ...
    def get_user_id_by_uuid(self, uuid: str) -> dict:
        ...
        """
        self.__validate_current_auth_token()
        res = self.__session.get(f"{self.__BASE_URL}admin/realms/{self.__REALM}/users/{uuid}", headers={'Authorization': 'Bearer ' + self.__token['token']})
        res.raise_for_status()
        return res.json()

    def update_user(self, uuid: str, data: dict) -> dict:
        """
        Updates a user in keycloak based on data given. Since keycloak does not support patical updates we have to get the current user object add our data then update.
        ...
        res = self.__session.put(f"{self.__BASE_URL}admin/realms/{self.__REALM}/users/{uuid}", headers={'Authorization': 'Bearer ' + self.__token['token'], "Content-Type": "application/json"}, json=user)
        res.raise_for_status()
    ...
    def authorize_redirect(self, redirect_uri, *args, **kargs) -> Response:
        """
        Wrapper for the poorly typed "oauth.register()" method to authorize a redirect we set the nonce here when we're given the session also redirect uri
        ...
        :returns: flask Response (must be final line if being used as it will redirect the user out of the current function)

        """
        if self.__oAtuh is None:
            raise EnvironmentError("oAuthManger.setup_oauth was not called yet cant use this.")

        nonce = token_urlsafe(16)
        session['nonce'] = nonce

        return oAuthManager.__oAtuh.authorize_redirect(redirect_uri=redirect_uri, nonce=nonce, *args, **kargs)

    def authorize_access_token(self) -> dict:
        """
        Wrapper for the poorly typed "oauth.authorize_access_token()" to get the user's login session after calling oAuthManager.authorize_redirect()
