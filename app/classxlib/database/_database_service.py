...
from .service import (UserService, UserLevelService,
                      OriginalImageService,CropImageService,
                      TrainingFileService,SegmentImageService,
                      LabelImageService, ResearchFieldService,
                      UserFriendService, UserResearchFieldService)
from .model import (User, UserLevel,
                    ResearchField)
...
class DatabaseService:
    ...
    def __init__(self, database_url:str, echo:bool=False):
        ...
        self.engine : Engine = self._create_db_engine(self.database_url,
                                                      echo=self.echo)
    ...
    def _create_db_engine(self,
        ...
        """
        try:
            return create_engine(database_url,pool_recycle=1800,pool_pre_ping=True, echo=echo)
    ...
    def _setup_services(self) -> bool:
        try:
            ...
            self.DEFAULT_ID = default_user_obj.id if default_user_obj is not None else None
            return True
        except (SQLAlchemyError, DatabaseError,
                    DBAPIError, RuntimeError) as error:
    ...
    def _validate_user_levels(self) -> bool:
        ...
        try:
            ...
            user_level_config_path = os.path.join(os.path.curdir,
                                             "classxlib/database/config/user_level.json")
            ...
            if os.path.isfile(user_level_config_path):
                ...
                with open(user_level_config_path,
                          'r',
                          encoding='utf-8') as user_level_config_file:
                    ...
                    for user_level_name in user_level_config.keys():
                        ...
                        if user_level_obj is None:
                            ...
                            user_level_obj = UserLevel(name=user_level_name,
                                                       permissions=user_level_permissions)

                            # Adding it to database
                            user_level_obj = self.user_level_service.add_level(user_level_obj)
                            continue

                        # Verifying the permissions are up to date
                        if dict(user_level_obj.permissions) != dict(user_level_permissions):
                            print(user_level_name,"permissions invalid, updating")
                            # Updating permissions if not
                            self.user_level_service.\
                                update_level_permissions(user_level_id=user_level_obj.id,
    ...
    def _validate_research_fields(self) -> bool:
        ...
        try:
            ...
            research_field_config_path = os.path.join(os.path.curdir,
                                                 "classxlib/database/config/research_field.json")
            # Verifying the file existance
            if os.path.isfile(research_field_config_path):
                print("Research field Config found verifying user levels")
                # Opening the config file
                with open(research_field_config_path,'r') as research_field_config_file:
                    # Loading the json data
                    ...
                    for research_field_name in research_field_config.keys():
                        ...
                        # Checking if they already exist
                        research_field_obj = self.research_field_service.\
                            get_default_field_by_name(research_field_name)

                        label_map = research_field_config[research_field_name]['label_map']
                        metadata_map = research_field_config[research_field_name]['metadata_map']
                        protocols = research_field_config[research_field_name]['protocols']

                        # If they are None it does not exist
                        if research_field_obj is None:
                            print(research_field_name,"does not exist adding to database")
                            # Creating a new UserLevel database object
                            research_field_obj = ResearchField(name=research_field_name,
                                                               user_id=None,
                                                               visibility=1,
                        ...
                        if research_field_obj.label_map !=\
                            ...
                            self.research_field_service.\
                                update_label_map(research_id=research_field_obj.id,
                        ...
                        if research_field_obj.metadata_map !=\
                            ...
                            self.research_field_service.\
                                update_metadata_map(research_id=research_field_obj.id,
                                                    new_metadata_map=metadata_map)
                        # Verifying the protocols are up to date
                        if research_field_obj.protocols !=\
                            research_field_config[research_field_name]['protocols']:

                            print(research_field_name,"protocols invalid, updating")

                            # Updating protocols if not
                            self.research_field_service.\
                                update_protocols(research_id=research_field_obj.id,
    ...
    def _validate_users(self) -> bool:
        ...
        try:
            ...
            default_field_id_list = [research_field_obj.id for research_field_obj\
                in self.research_field_service.get_default_fields()]
            ...
            if default_user_obj is None:
                ...
                default_user_obj = User(username="default",
                                        kc_uuid="temp",
                ...
                if default_user_obj.kc_uuid == "temp":
                    print("The default user is not set up in keycloak.",
                ...
                added_research_fields = [added.research_id for added in self.user_research_fields_service.get_user_research_fields(_id)]
                ...
                for research_field_id in default_field_id_list:
                    if research_field_id not in added_research_fields:
                        print(f"Research Field ID {research_field_id} missing from default user adding now")
                        self.user_research_fields_service.add_research_field(default_user_obj.id, research_field_id, 999)
