...
def read_cv_image(path:str,
                  reverse_channel:bool=True,
                  flag:int=cv2.IMREAD_ANYCOLOR,
                  noflag:bool=False) -> np.ndarray:
    ...
    try:
        ...
        if reverse_channel is True:
            ...
            cv_image = cv2.cvtColor(cv_image,
                                    cv2.COLOR_BGR2RGB) if len(cv_image.shape) == 3 else cv_image
        else:
            cv_image = cv2.imread(path) if noflag is True else cv2.imread(path, flag)
        return cv_image
...
def read_hdf5_image(path:str,
    ...
    try:
        ...
        if path.endswith(".h5"):
            ...
            image_array = h5_file[dataset_name][:]
...
def read_fits_image(path:str,
    ...
    if image_header['LVL_NUM'] < 1.5:
        sunpy_map = Map((image_data,image_header))
        pointing_table = get_pointing_table("JSOC", time_range=(sunpy_map.date - 12 * u.h, sunpy_map.date + 12 * u.h))# Create Sunpy Map
        sunpy_map = update_pointing(sunpy_map, pointing_table=pointing_table)      # Update Header based on Latest Information
