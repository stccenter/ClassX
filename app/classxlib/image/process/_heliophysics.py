...
def _process_heliophysic_image(path:str,research_field_obj:ResearchField):
    image_data, image_header, image_map = read_fits_image(path=path,
                                                          index=1)
    print("Heliophysic Image Data Read")
    processed_data,im_size,sun_radius,sun_center = _resize_EUV(image_data,image_header,2)
    ...
    image_data_dict['h5'] = rescale_intensity(processed_data,
...
def _process_visual_image(input_image:np.ndarray, image_dim:tuple):
    ...
    input_image = resize_image(input_image=input_image,
...
def _process_metadata(image_map):
    ...
    metadata_dict = {"longitude":float(image_map.carrington_longitude.deg),
...
def _resize_EUV(J,h,resize_param=8,interpolation='Bi-cubic'):
    ...
    # Resize image
    if resize_param > 1:
        I = resize(J,np.asarray(J.shape)/resize_param,
