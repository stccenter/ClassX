...
def process_image_grid(input_image:np.ndarray,
                    crop_size:int=256,
                    grid_line_color:str='#b1ddfc',
                    line_thickness:int=16,
                    draw_lines:bool=True) -> np.ndarray:
    ...
    try:
        ...
        image_target_width, image_target_height = input_image.shape[1], input_image.shape[0]
        ...
        grid_image = _create_grid_image(input_image=input_image,
                                     crop_size=crop_size,
...
def _create_grid_image(input_image:np.ndarray,
                    crop_size:int=256,
    ...
    try:
        ...
        # Padding the image
        padded_image = pad_image(input_image=input_image,
        ...
        if draw_lines is True:
            # draw vertical lines
            for x in np.linspace(start=delta_x,
                                 stop=target_x_dim-delta_x,
                                 num=grid_x_count-1):
                cv2.line(img=padded_image,
                         pt1=(int(round(x)), 0),
            ...
            for y in np.linspace(start=delta_y,
                                 stop=target_y_dim-delta_y,
                                 num=grid_y_count-1):
                cv2.line(img=padded_image,
                         pt1=(0, int(round(y))),
...
def process_grid_crop_square(input_image:np.ndarray,original_adjusted_image:np.ndarray,grid_coord, tile_shape, stride_shape):

    x_index = grid_coord[0]
    y_index = grid_coord[1]
    x = x_index * stride_shape[0]
    y = y_index * stride_shape[1]
    crop_image, original_crop_image = createCrop(preprocessed_image, original_adjusted_image, x, y, tile_shape[0])
