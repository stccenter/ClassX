...
def create_user_directories(base_dir : str, user_name : str) -> bool:
    ...
    try:
        ...
        input_directory = merge_directory(base_dir,
                                          user_name+'/ReadGUI')

        # Where the processed original image data is stored
        processed_directory = merge_directory(base_dir,
                                              user_name+'/ProcessedImages')

        # Where all active writing directories are stored
        output_directory = merge_directory(base_dir,
                                           user_name+'/WriteGUI')

        # Where training datasets are exported to before zipping them
        export_directory = merge_directory(base_dir,
                                           user_name+'/export')

        # Directories to verify
        required_directories = [base_dir,
                                input_directory,
                                processed_directory,
                                output_directory,
                                export_directory]
        ...
        for directory in output_sub_directories:
            required_directories.append(merge_directory(output_directory,
                                                         directory))
