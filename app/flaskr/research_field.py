...
from classxlib.database.service import ResearchFieldService, UserService, UserResearchFieldService
...
@RESEARCH_FIELD.route('/getDefaultResearchFields',methods=['POST','GET'], endpoint="getDefaultResearchFields")
...
@RESEARCH_FIELD.route('/switchResearchField',methods=['POST','GET'], endpoint="switchResearchField")
def switch_research_field():
    ...
    try:
        ...
        if not user_research_fields_service.in_research_id(user_obj.id, new_research_id):
            return{'status':400,'current_research_field':{}, 'error':"User does not have access to this research field"}
        ...
        # Appending the upload time and creation date to the metadata map of the current research field since they are columns that don't exist in the normal map
        new_research_field_obj.metadata_map['upload_time'] = {'type':'date', 'data':{'startdate':'-1', 'enddate':'-1'}}
        new_research_field_obj.metadata_map['creation_date'] = {'type':'date', 'data':{'startdate':'-1', 'enddate':'-1'}}

        # Formatting the return dict for front end
        current_research_field = {"id":new_research_field_obj.id,
...
@RESEARCH_FIELD.route('/getUserResearchFields',methods=['POST','GET'], endpoint="getUserResearchFields")
def get_user_research_fields():
    ...
    try:
        research_id_list = [field.research_id for field in user_research_fields_service.get_user_research_fields(user_obj.id)]

        # User's associated research field list from the
        # research field ids stored on the account.
        research_field_obj_list = research_field_service.\
            get_by_ids(research_id_list)

        # User ResearchField List
        research_field_list = [{"id":research_field.id,
                                "name":research_field.name,
                                "label_map":research_field.label_map,
                                "metadata_map":research_field.metadata_map,
                                "grid_size":research_field.protocols['auto_grid_size']} for research_field in research_field_obj_list]
        ...
        return {'status':200,'current_research_field':current_research_field,'research_fields':research_field_list}
