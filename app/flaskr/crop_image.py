...
from classxlib.image import read_cv_image, read_hdf5_image, write_cv_image, write_hdf5_image
...
from classxlib.database.service import (CropImageService,OriginalImageService,
                                        UserService, SegmentImageService,
                                        LabelImageService, TrainingFileService)
...
from .globals import (USER_UPLOAD_FOLDER, IMAGE_FOLDER,
                      STATIC_FOLDER)
...
def save_crop_image():
    """API ENDPOINT
     ...
     Request Args:
        ...
        request_dict(dict): Parameters for cropping stored in json
    ...
    request_dict = {}
    try:
        # Getting the request data from front-end
        ...
        click_location_x = int(request_dict['xNavName_'+original_image_id])
        click_location_y = int(request_dict['yNavName_'+original_image_id])
        crop_size = int(request_dict['rangeZoomName_'+original_image_id])
    ...
    print(f'User clicked on {original_image_obj.name} at {click_location_x} {click_location_y} span dimension {crop_size} {crop_size}')

    # Setting the correct file paths for later
    if is_default_user(user_obj):
        crop_file_path = merge_directory(IMAGE_FOLDER, user_obj.username+'/WriteGUI/CropImages')
    else:
        crop_file_path = merge_directory(USER_UPLOAD_FOLDER, user_obj.username+'/WriteGUI/CropImages')
    ...
    try:
        ...
        visual_original_image_path = merge_directory(STATIC_FOLDER, original_image_obj.visualization_path)
        ...
        if original_image_obj.h5_path is not None:
            h5_original_image_path = merge_directory(STATIC_FOLDER, original_image_obj.h5_path)
        ...
        crop_image, h5_crop_image = create_crop(visual_image=visual_image,
        ...
        crop_image_obj = CropImage(name=crop_file_name,
                                   user_id=user_obj.id,
                                   shared_by=None,
                                   shared_from=None,
        ...
        return {'status': 200, 'crop_image' : crop_image_obj, 'original_image':original_image_obj}
...
@CROP_IMAGE.route('/deleteCropImage', methods=[ 'GET', 'POST'], endpoint="deleteCropImage")
def delete_crop_image():
    """API ENDPOINT
        Deletes a crop image from a user account. Deletes all related segment images as well and then removes files.

    Session Args:
        username(str): Username retrieved from the current session.

    Request Args:
        crop_image_id(int): The id of the crop image to delete retrieved from front-end request.

    Returns:
        dict: Formatted return dict object for front-end, converted to json during during return contents.
            status(int): HTTP Request status code. Returns 200 if successful or 400 if there is authorization issues and/or errors.
            message(str): Informational message for user. Only returned if status code is 200.
            error(str): Informational error message for user. Only returned if status code is a 400 series code.
    """
    # Retrieving Database
    db = get_db()
    oauth = get_oauth()
    ...
    crop_image_obj = crop_image_service.get_user_image(crop_image_id, user_id=user_obj.id, default_id=db.DEFAULT_ID)

    # Verifying user has access to delete this image
    if crop_image_obj.user_id != user_obj.id and user_obj.user_level != 3:
        return {'status':400, 'error':"You do not have authorization to delete this image"}
    ...
    try:
        ...
        if crop_image_obj.shared_by is not None:
            ...
            return{'status':200, 'message':"This image was shared so we have removed your access. \
            ...
            segment_image_obj_list = segment_image_service.get_images_from_parent_image(crop_image_obj.id)
            ...
            for segment_image_obj in segment_image_obj_list:
                ...
                label_image_obj_list = label_image_service.get_images_from_parent_image(segment_image_obj.id)
                ...
                if len(label_image_obj_list) != 0:
                    ...
                    for label_image_obj in label_image_obj_list:
                        training_file_obj = training_file_service.get_file(label_image_obj.training_file_id)
                        message = message + str(training_file_obj.file_name) + " "

                    # Informing user which datasets to remove image from before continuing
                    return{'status':400, 'error':"Please remove this image from the \
                        following training files before deleting: " + message}
                else:
        ...
        traceback.print_tb(error.__traceback__)
        return{'status':400,'error':"Error deleting image. Please try again. If issue persists please consult an admin for help"}
...
def getUserCropImage():
    """API ENDPOINT
    Retrieves a cropped image by its ID for the current user.

    Session Args:
        user_obj(User): User database object retrived by
    ...
    # Getting the associated crop image id
    crop_image_obj = crop_image_service.get_user_image(crop_image_id,
                                                       user_id=user_obj.id,
                                                       default_id=db.DEFAULT_ID)
...
@CROP_IMAGE.route("/getUserCropImages", methods=['GET','POST'], endpoint='getUserCropImages')
def getUserCropImages():
    """API ENDPOINT
    ...
    if request.method == 'GET':
        crop_image_obj_list = crop_image_service.get_user_research_images(research_field_id=session['research_field']['id'],
                                                                          user_id=user_obj.id,
                                                                          crop_type='man')
        original_image_obj_list = original_image_service.get_user_research_images(research_field_id=session['research_field']['id'],
                                                                                  user_id=user_obj.id,
                                                                                  default_id=db.DEFAULT_ID)
        return render_template('segment.html', crop_images = crop_image_obj_list, original_image=original_image_obj_list)
...
@CROP_IMAGE.route("/getAutoCropImage/", methods=['GET','POST'], endpoint="getAutoCropImage")
def get_autocrop_image():
    ...
    # Note this may be different from the original image dimensions depending on if part of the grid was cropped or padded to meet crop sizes
    ...
    # The grid position is now the actual crop point from the image. The crop point is the bottom right corner of a cropped image.
    # The auto grid size is added on at the end due to it being the bottom right corner.
    grid_position_x = (grid_position_x * research_field['grid_size']) + research_field['grid_size']
    grid_position_y = (grid_position_y * research_field['grid_size']) + research_field['grid_size']

    # Getting the database object for the cropped image by matching the original image id and crop point.
    # This function only retrieves 'auto'(automatic) cropped images to avoid crop point overlaps with 'man'(manual) cropped images

    crop_image_obj = crop_image_service.get_user_image_from_grid(original_image_id=original_image_id,
    ...
    if crop_image_obj is None:
        ...
        try:
            ...
            if original_image_obj.shared_from is not None:
                original_image_obj = original_image_service.get_image(original_image_obj.shared_from)

            auto_crop_image.delay(original_image_obj.id, research_field['grid_size'])
            #auto_crop_request = Process(target=auto_crop_image, args=(user_obj.id ,original_image_id, research_field['grid_size']))
            #auto_crop_request.start()
        except Exception as error:
            print("Auto Cropping TASK FAILED")
            traceback.print_tb(error.__traceback__)
        return{'status': 404,'error':'Image not found, verifying crop grid. Please try again in a few minutes.'}

    if crop_image_obj.user_id != user_obj.id and crop_image_obj.user_id != db.DEFAULT_ID:
