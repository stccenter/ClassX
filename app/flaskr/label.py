...
from classxlib.database.service import (
    CropImageService,
    LabelImageService,
...
from classxlib.image._write import write_hdf5_image
...
def load_label_tool():
    ...
    crop_image_obj = crop_image_service.get_user_image(crop_image_id=crop_image_id,
                                                       user_id=user_obj.id,
    ...
    training_file_obj_list = training_file_service.get_user_research_files(research_field_id=research_field_obj.id,
                                                                           user_id=user_obj.id,
                                                                           default_id=db.DEFAULT_ID)


    # Getting all the ids of the uploaders
    uploader_ids = list(map(lambda segment_image_obj: segment_image_obj.shared_by, segment_image_obj_list))

    # Setting the names of the uploader based off the name
    uploader_names = list(map(lambda user_id: user_service.get_by_id(user_id).name if user_id is not None else user_obj.username, uploader_ids))

    # Formatting a list of the dates created
    date_created_list = list(map(lambda x: datetime.strptime(x.name[12:30],'%m_%d_%Y_%H_%M_%S').date(), segment_image_obj_list))
...
@LABEL.route("/updateLabelOpacity/", methods = ['GET', 'POST'], endpoint="updateLabelOpacity")
def update_label_opacity():
    """
    API ENDPOINT Retrieves the updated segmented image's boundaries
    ...
    segment_image_obj = segment_image_service.get_user_image(segment_image_id=segment_image_id,
                                                             user_id=user_obj.id,
                                                             default_id=db.DEFAULT_ID)
    ...
    for label in research_field_obj.label_map:
        research_field_label_map[label["id"]] = {"name":label["name"],"color":label["color"]}

    # Gettting the counts of labeled and unlabeled segments
    total_segment_count, labeled_segment_count, unlabeled_segment_count = get_labeled_segment_count(segment_info)

    # Formatting path to marked image
    marked_image_path = merge_directory(STATIC_FOLDER,segment_image_obj.marked_image_path)
    ...
    if unlabeled_segment_count == total_segment_count:
        ...
        color_image = color_labeled_image(input_image=marked_image,
...
def get_label_image():
   ...
   Returns:
        JSON:Formatted response dict object for front-end
            ...
            - label_class_list(list): List of tuples containing label IDs and their respective counts.
    ...
    research_label_map = {}
    for label in research_field_obj.label_map:
        research_label_map[label["id"]] = {"name":label["name"],"color":label["color"]}
    ...
    return {'status': 200,
            ...
            'label_class_list': np.column_stack(np.unique(segment_info[:,1], return_counts=True)).tolist(),
            'label_map':research_label_map}

######################################################################
# endpoint called when segmented image is clicked in labeling process
######################################################################
#TODO: we need to update the charts here onclick so when user click a segment to relabel, chart would update, same to all 3 endpoints
...
def label_segment():
    ...
    Request Args:
        segment_image_id(int): The id of the segmented image.
        domain_label_id(int): The id of the domain label
    ...
    actualSize = request.args.get('actualSize',type=int)
    ...
    adjusted_location_x = int((segment_image_obj.crop_size/actualSize) * click_location_x)
    adjusted_location_y = int((segment_image_obj.crop_size/actualSize) * click_location_y)
...
######################################################################
@LABEL.route("/setUnlabeledUnknown/", methods = ['GET', 'POST'], endpoint="setUnlabeledUnknown")
def set_unlabeled_unknown():
    ...
    if remove_small:
        ...
        segment_info = remove_small_labels(segment_image, segment_info, unique_label_id_list, area_removal_percentage, unknown_label_id)
        update_segment_image_info(segment_info, segment_image_path)
    ...
    segment_info = np.column_stack((segment_numbers, segment_labels, segment_area_count))
...
def get_label_area():
    ...
    if segment_image_obj is None:
        segment_image_obj = segment_image_service.get_image(segment_image_id=segment_image_id)
    ...
    for label_id in unique_label_ids.tolist():
        ...
        label_area_dict[label_id] = int(np.sum(np.where(segment_info[:,1] == label_id, segment_info[:,2], 0)))
...
def check_segment_label():
    ...
    if unlabeled_segment_count != 0:
        ...
        return {'status': 400,
                'error': "Not all segments filled. Save incomplete segments in 'Unknown' class by clicking OK."}

    return {'status': 200}

@LABEL.route("/saveTrainingFile/", methods = ['GET', 'POST'], endpoint="saveTrainingFile")
def save_training_file():
    ...
    # Save Type for the training file if 1 then creating a new file, if 2 we are appending to an existing dataset
    ...
    crop_image_obj = crop_image_service.get_image(crop_image_id=segment_image_obj.crop_image_id)

    # Getting the original image associated with crop image
    original_image_obj = original_image_service.get_image(original_image_id=crop_image_obj.original_image_id)
    ...
    print(f'Segmented image {segment_image_obj.name} {segment_image_obj.id} is being processed {segment_image_obj.segment_path}.')
    
    # Remove small labels TEMP REMOVED SINCE I WAS TOLD IT SHOULD BE 
    if remove_small:
        print('Removing small labels...')
        unknown_label_id = get_unknown_label_from_research_field(research_field_obj)
        unique_label_id_list: np.ndarray = np.unique(segment_info[:,1])
    ...
    if is_default_user(user_obj):
        if file_type == 'HDF5':
            file_path= merge_directory(ADMIN_UPLOAD_FOLDER, user_obj.username+'/WriteGUI/HDF5')
    else:
        if file_type == 'HDF5':
            file_path = merge_directory(USER_UPLOAD_FOLDER, user_obj.username+'/WriteGUI/HDF5')

    # This is a future proof function incase there is additional datasets supported it will ensure the filepath for them exists
    ...
    try:
        ...
        if save_type == '1':
            if file_type == 'HDF5':
                ...
                training_file_obj = training_file_service.get_user_file_by_name(training_file_name=training_dataset_filename,
                                                                                user_id=user_obj.id)
                if training_file_obj is not None:
                    return {'status': 400, 'alert': alert, 'error':"File already exists under that name"}

                # Formatting the save path for the training file
                training_file_save_path = merge_directory(file_path, training_dataset_filename)

                # Creating the HDF5 file
                write_training_file(training_file_save_path,
                                    STATIC_FOLDER,
                                    segment_image_obj,
                                    crop_image_obj,
                                    original_image_obj,
                ...
                training_file_database_path = format_database_path(file_path+'/'+training_dataset_filename)
                model_database_path = format_database_path(file_path)

                # save the numbers as 0 for now, we will alter later
                training_file_obj = TrainingFile(user_id=user_obj.id,
            ...
            if file_type == 'HDF5':
                ...
                if training_file_obj is None:
                    return {'status': 400, 'alert': alert, 'error':"User does not have permission to edit this dataset"}

                if training_file_obj.shared_from is not None:
                    training_file_obj = training_file_service.get_file(training_file_obj.shared_from)
                ...
                training_file_path = merge_directory(file_path, training_dataset_filename)

                #Creating the HDF5 file
                write_training_file(training_file_path, STATIC_FOLDER, segment_image_obj, crop_image_obj, original_image_obj, overwrite=False)

        # Getting the visual image
        visual_image_path = merge_directory(STATIC_FOLDER,crop_image_obj.visualization_path)
        visual_image = read_cv_image(visual_image_path)

        # Colored label image
        color_image = color_image = color_labeled_image(input_image=visual_image,
                                          segment_image=segment_image,
                                          segment_info=segment_info,
                                          research_label_map=research_field_obj.label_map,
                                          alpha=0.7)
        ...
        if is_default_user(user_obj):
            color_image_path = merge_directory(ADMIN_UPLOAD_FOLDER,
                                               user_obj.username+'/WriteGUI/ColorImages')
        else:
            color_image_path = merge_directory(USER_UPLOAD_FOLDER,
                                               user_obj.username+'/WriteGUI/ColorImages')

        # Name of the color image file
        color_image_file_name = str(training_file_obj.id) + "_" + str(segment_image_id) +"_color_image.png"
        ...
        label_image_obj = label_image_service.get_image_from_parents(segment_image_id=segment_image_id,
                                                                     training_file_id=training_file_obj.id)

        # Check if the file already exists
        if label_image_obj is not None:
            # Remove old color image
            old_color_image_path = merge_directory(STATIC_FOLDER, label_image_obj.color_image_path)
            ...
            label_image_service.update_last_modified(segment_image_id=segment_image_id,
                                                     training_file_id=training_file_obj.id,
                                                     new_date=date)
        else:
            ...
            label_image_obj = LabelImage(user_id=training_file_obj.user_id,
                                         training_file_id=training_file_obj.id,
...
@LABEL.route("/autoLabelImage/", methods = ['GET','POST'], endpoint="autoLabelImage")
def auto_label_image():
    ...
    training_file_obj = training_file_service.get_file(training_file_id=training_file_id)
    ...
    # Another compatibility feature since tif images are originally in the shape (3,x,y) unlike standard images which are (x,y,3)
    ...
    research_field_obj = research_field_service.get_by_id(research_id=segment_image_obj.research_id)
    research_label_map = research_field_obj.label_map
    ...
    model_file_name = str(training_file_obj.id) +"_"+ training_file_obj.file_name.split(".")[0] + "_" +  model_mapping[algorithm_id] + ".pkl"
    ...
    training_dataset = read_training_file(target_training_file, image_type, int(unknown_label_id))

    # classify image
    classified_flags = classify_image(cropped_image_reshape, segment_image, training_dataset, \
                                                    [image_type, research_field_obj.name], probability_threshold, algorithm_id, \
                                                    model_file_path, needs_retrain)
...
@LABEL.route('/deleteImageFromTrainingFile/', methods=[ 'GET', 'POST'], endpoint="deleteImageFromTrainingFile")
def delete_image_from_training_file():
    ...
    training_file_obj = training_file_service.get_user_file(training_file_id=training_file_id,
                                                            user_id=user_obj.id)
...
@LABEL.route('/shareTrainingFileToUsers/', methods=[ 'POST'], endpoint="shareTrainingFileToUsers")
def share_training_file_to_users():
    ...
    if training_file_obj.shared_from is not None and (training_file_obj.user_id != user_obj.id and user_obj.user_level != 3):
        return {'status': 404, 'error': "User does not have access to share this file"}

    if len(username_share_list) <= 0: return {'status': 404, 'error': "Recipient list is empty"}
    ...
    for username in username_share_list:
        try:
            ...
            share_training_file_obj = TrainingFile(user_id=user_service.get_by_username(username).id,
    ...
    return {'status': 200, 'failed_list': failed_recipients, 'passed_list': passed_recipient}

@LABEL.route('/getTrainingFileContents/', methods=['GET'], endpoint="getTrainingFileContents")
def view_training_file():
   ...
   # read label file and get all original image, crop image, segmented image
    training_file_obj = training_file_service.get_user_file(training_file_id,
                                                            user_id=user_obj.id,
                                                            default_id=db.DEFAULT_ID)
    if training_file_obj is None:
    ...
    if ".h5" in training_file_obj.file_name:
        ...
        segment_image_id_list,crop_image_id_list, original_image_id_list = \
            get_unique_parent_ids_from_link(segment_id_link,
                                            return_crop_id=True,
                                            return_original_id=True)
        ...
        original_image_obj_list = original_image_service.get_images(original_image_id_list)

        label_image_obj_list = label_image_service.get_images_from_parent_file(training_file_id)
        return render_template('viewlabeledfiles.html',
...
@LABEL.route('/getTrainingFiles/', methods=['GET'], endpoint="getTrainingFiles")
def get_training_files():
    ...
    training_file_obj_list = training_file_service.get_user_research_files(research_field_id=research_field_id,
                                                                           user_id=user_obj.id,
                                                                           default_id=db.DEFAULT_ID)

    return {'status': 200, 'training_files': training_file_obj_list}

@LABEL.route('/viewTrainingFiles/', methods=['GET'], endpoint="viewTrainingFiles")
def view_training_files():
    ...
    training_file_obj_list = training_file_service.get_user_research_files(research_field_id=research_field["id"],
                                                                           user_id=user_obj.id,
                                                                           default_id=db.DEFAULT_ID)

    file_uploader_id_list = list(map(lambda training_file_obj:\
        training_file_obj.shared_by if training_file_obj.shared_by is not None\
            else training_file_obj.user_id, training_file_obj_list))
    uploader_names = list(map(lambda user_id:\
        user_service.get_by_id(user_id).username\
            if user_id is not None\
                else user_obj.username, file_uploader_id_list))

    last_modified_dates = list(map(lambda training_file_obj:\
        label_image_service.get_last_modified_from_parent_file(training_file_obj.shared_from) \
            if training_file_obj.shared_from is not None \
                else label_image_service.get_last_modified_from_parent_file(training_file_obj.id), training_file_obj_list))

    last_modified_dates = [label_image_obj.last_modified.replace(tzinfo=timezone.utc)\
        if label_image_obj is not None\
            else datetime.now(timezone.utc) for label_image_obj in last_modified_dates]

    combined_data = list(zip(training_file_obj_list,last_modified_dates,uploader_names))
    sorted_data = sorted(combined_data, key=lambda x: x[1], reverse=True)
    return render_template('view.html',  sorted_data = sorted_data)

@LABEL.route("/exportTrainingImages/", methods=["GET"], endpoint="exportTrainingImages")
def export_training_images():
    ...
    Session Args:
        user_obj(User): User database object retrieved by verifying session token against database.
    ...
    training_file_id = request.args.get("training_file_id", type=int) # For front-end requests
    ...
    if is_default_user(user_obj):
        EXPORT_DIR_PATH = merge_directory(ADMIN_UPLOAD_FOLDER, f"{user_obj.username}/export")
    else:
        EXPORT_DIR_PATH = merge_directory(USER_UPLOAD_FOLDER, f"{user_obj.username}/export")
    ...
    if visual_image_check == 1:
        FULL_EXPORT_VISUAL_PATH = merge_directory(FULL_TRANING_DIR_PATH, "visual")
    ...
    label_vector, _, segment_id_link = read_training_file(training_file_path, 'srgb', return_id_link=True)
    ...
    for segment_image_id in unique_segment_id_list:
        ...
        original_image_obj = original_image_service.get_image(
            original_image_id=specified_segment[0,4] # original image id (row 0, col 4)
        )
        ...
        # Cropped image name
        cropped_image_name, _ = os.path.splitext(crop_image_obj.name)
        ...
        try:
            image_file_type = "png" if "png" in user_image_file_type else user_image_file_type
            image_file_name = f"image_{cropped_image_name}.{image_file_type}"
            export_image_path = merge_directory(FULL_EXPORT_IMAGES_PATH, image_file_name)
            export_image_file_type(crop_image_h5_path, export_image_path, user_image_file_type)
            export_paths["image_path"] = merge_directory(TRAINING_FILE_NAME, f"images/{image_file_name}")
        except Exception as error:
            print(error)
            traceback.print_tb(error.__traceback__)
            return make_response(('',404, {'error': "Crop image could not be converted"}))
        
        # Copies cropped images (visuals) onto the export directory
        try:
            if visual_image_check == 1:
                crop_image_visual_path = merge_directory(STATIC_FOLDER, crop_image_obj.visualization_path)
                ...
                export_paths["visual_path"] = merge_directory(TRAINING_FILE_NAME, f"visual/visual_{file_name}")
            else:
        ...
        try: 
            export_as_coco(crop_image_obj.id, label_mask, master_dict, unique_label_id_list, segment_image_id)
        except Exception as error:
            print(error)
            traceback.print_tb(error.__traceback__)
        ...
        try:
            ...
            mask_file_type = "png" if "png" in user_mask_file_type else user_mask_file_type
            mask_file_name = f"mask_{cropped_image_name}.{mask_file_type}"
            ...
            export_paths["mask_path"] = merge_directory(TRAINING_FILE_NAME, f"masks/{mask_file_name}")
        except Exception as error:
            print(error)
            traceback.print_tb(error.__traceback__)
    ...
    with open(f"{FULL_TRANING_DIR_PATH}/coco_{TRAINING_FILE_NAME}.json", 'w') as jsonObj:
        json.dump(master_dict, jsonObj, indent=4)
        jsonObj.close()  

    # Attempts to create zip folder
    try:
        export_zip_file_path = shutil.make_archive(FULL_TRANING_DIR_PATH, "zip", FULL_TRANING_DIR_PATH)
    except Exception as error:
        print(error)
        traceback.print_tb(error.__traceback__)
    ...
    response =  send_file(file_buffer,
                          download_name=export_file_zip_name,
                          as_attachment=True)
