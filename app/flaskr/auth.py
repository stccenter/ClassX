...
def login():
    ...
    return oauth.authorize_redirect(redirect_uri=url_for('auth.callback',_external=True))
...
def callback():
    ...
    Returns:
        redirect: Redirects to kc_index to login in the
...
@AUTH.route('/logInUser', methods=['GET','POST'], endpoint="logInUser")
def log_in_user():
    """A middle man API to set up the session info
    and user data in the flask server. Verifies the
    user is in the session then verifies the user directories
    before redirecting to the dashboard.

    Returns:
        redirect: Redirects to the dashboard if the user is in session
    ...
    if 'user' in session:
        ...
        if user_obj is None:
            user_obj = user_service.get_by_username(token_obj['userinfo']['preferred_username'])
            if user_obj is not None and token_obj['userinfo']['sub'] != user_obj.kc_uuid:
                user_obj = user_service.update_uuid(user_id=user_obj.id,
                                                    new_uuid=token_obj['userinfo']['sub'])
            else:
                if token_obj['userinfo']['preferred_username'] == 'default':
                    ...
                    if default_user_obj.kc_uuid == "temp":
                        user_obj = user_service.update_uuid(user_id=default_user_obj.id,
                                                            new_auth_token=token_obj['userinfo']['sub'])
                else:
                    # Getting a list of all the default research field ids
                    research_field_id_list= [research_field_obj.id for research_field_obj\
                        in research_field_service.get_default_fields()]

                    # The newly created user object
                    new_user_obj = User(username=token_obj['userinfo']['preferred_username'],
                    ...
                    user_research_objs = [UserResearchField(user_obj.id, _id, 0) for _id in research_field_id_list]
        ...
        if 'url' in session:
            ...
            return redirect(url_for('auth.redirectPrevious'))
