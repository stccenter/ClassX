...
def dashboard():
    ...
    Args:
        ...
        index_request(dict): Contains the search filters and query. Will be none unless POST Request.
    ...
    # If the session domain doesn't exist set it to default associated domain
    if current_research_field is None:
        user_research_fields_service = db.user_research_fields_service

        # Retrieving the user's associated research fields
        assocaited = user_research_fields_service.get_user_research_fields(user_obj.id)

        # TODO: Remove this and run a proper migration script...
        if assocaited is None or len(assocaited) == 0:
            ...
            research_field_obj = research_field_service.get_by_id(assocaited[0].research_id)

        # Formatting the return dict for front end
        current_research_field = {"id":research_field_obj.id,
        ...
        research_field_obj = research_field_service.\
            get_by_id(session['research_field']["id"])
    ...
    research_field_obj.metadata_map['upload_time'] = {'type':'date',
                                                      'data':{'startdate':'-1', 'enddate':'-1'}}
    research_field_obj.metadata_map['creation_date'] = {'type':'date',
                                                        'data':{'startdate':'-1', 'enddate':'-1'}}
    ...
    try:
        ...
        image_filters = index_request["image_filters"] if "image_filters" in index_request else None

        # Retrieving Images from Database with filters
        original_image_obj_list = original_image_service.search_images(query=search_query,
                                                                       user_id=user_obj.id,
        ...
        # This formatting is mainly for setting the uploader names otherwise we could just return the object list
        return_object = [{"id" : original_image_obj.id,
                          "name": original_image_obj.name,
                          "uploader_name" : user_service.\
                              get_by_id(original_image_obj.user_id).username,
                          ...
                          "thumbnail_path":original_image_obj.thumbnail_path} for \
                              original_image_obj in original_image_obj_list]
        ...
        if request.method == "GET":
            ...
            if len(original_image_obj_list) == 0:
                ...
                return {"status" : 200, "images" : return_object, 'research_field' : research_field_obj} # update
...
@DASHBOARD.route('/dashboard/<int:original_image_id>/segment/<int:crop_image_id>', methods=['GET', 'POST'])
def dashboard_segment(original_image_id:int,crop_image_id:int):
    ...
    # Returns 200 to signify everything ran correctly
    return  render_template('dashboard/segment.html',
                            original_id = original_image_id,
                            crop_image_id = crop_image_id,
                            original_image = original_image_obj,
                            crop_image = crop_image_obj)

@DASHBOARD.route('/dashboard/<int:original_image_id>/crop', methods=['GET', 'POST'])
def dashboard_crop(original_image_id:int):
    """API ENDPOINT
    Function to get all the cropped images associated with a User. By default will also get the default user crop images.
    ...
    max_crop_size = min(original_image_obj.height-original_image_obj.height%256,
    ...
    if request.method == 'GET':
        ...
        crop_image_obj_list = crop_image_service.\
            get_user_images_from_parent(original_image_id=original_image_id,
                                        user_id=user_obj.id,
                                        default_id=db.DEFAULT_ID,
                                        crop_type='man')
...
@DASHBOARD.route("/dashboard/label", methods=['GET', 'POST'], endpoint='/dashboard/label')
def dashboard_label():
    # Retrieving Database
    db = get_db()
    oauth = get_oauth()
    ...
    # Getting all images associated with crop image
